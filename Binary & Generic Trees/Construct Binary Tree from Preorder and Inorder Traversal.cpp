...
// using map to find element in inOrder array
...
map<int, int> &valueToIndexMap) {
  ...
  TreeNode* root = new TreeNode(preOrderFirstElement);
  // finding root in inOrder
  int position = valueToIndexMap[preOrderFirstElement]; // -> O(1)
  // int position = searchInorder(inOrder, preOrderFirstElement, size); //-> O(n)
  //recursion handles the rest
  root->left = constructTreeFromPreAndInorderTraversal(preOrder, preOrderIndex, inOrder, inOrdeStart, position - 1, size, valueToIndexMap);
  root->right = constructTreeFromPreAndInorderTraversal(preOrder, preOrderIndex, inOrder, position + 1, inOrderEnd, size, valueToIndexMap);
  return root;
}
...
int main() {
  ...
  TreeNode* root = constructTreeFromPreAndInorderTraversal(preOrder, preOrderIndex, inOrder, inOrdeStart, inOrderEnd, size, valueToIndexMap);
  printLevelWise(root);
  return 0;
}
